package com.nyuppo.entity.vehicle;
...
public class RaftEntity extends Entity {
    ...
    private static final TrackedData<Integer> RAFT_TYPE;
    ...
    private RaftEntity.Location location;
    private RaftEntity.Location lastLocation;
    ...
    public RaftEntity(EntityType<? extends RaftEntity> entityType, World world) {
    ...
    public RaftEntity(World world, double x, double y, double z) {
        this(ModEntityTypes.RAFT, world);
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(RAFT_TYPE, Type.BAMBOO.ordinal());
    ...
    public void pushAwayFrom(Entity entity) {
        if (entity instanceof RaftEntity) {
    ...
    public void tick() {
        ...
        if (this.location != RaftEntity.Location.UNDER_WATER && this.location != RaftEntity.Location.UNDER_FLOWING_WATER) {
    ...
    private void updatePositionAndRotation() {
        ...
        if (this.field_7708 > 0) {
            ...
            double g = MathHelper.wrapDegrees(this.raftYaw - (double)this.getYaw());
            this.setYaw(this.getYaw() + (float)g / (float)this.field_7708);
            this.setPitch(this.getPitch() + (float)(this.raftPitch - (double)this.getPitch()) / (float)this.field_7708);
            --this.field_7708;
    ...
    private RaftEntity.Location checkLocation() {
        RaftEntity.Location location = this.getUnderWaterLocation();
        if (location != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return location;
        } else if (this.checkRaftInWater()) {
            return RaftEntity.Location.IN_WATER;
        } else {
            float f = this.getNearbySlipperiness();
            if (f > 0.0F) {
                this.nearbySlipperiness = f;
                return RaftEntity.Location.ON_LAND;
            } else {
                return RaftEntity.Location.IN_AIR;
            }
        }
    }
    ...
    private boolean checkRaftInWater() {
        Box box = this.getBoundingBox();
        int i = MathHelper.floor(box.minX);
        int j = MathHelper.ceil(box.maxX);
    ...
    @Nullable
    private RaftEntity.Location getUnderWaterLocation() {
        Box box = this.getBoundingBox();
                ...
                for(int q = m; q < n; ++q) {
                    ...
                    if (fluidState.isIn(FluidTags.WATER) && d < (double)((float)mutable.getY() + fluidState.getHeight(this.world, mutable))) {
                        if (!fluidState.isStill()) {
                            return RaftEntity.Location.UNDER_FLOWING_WATER;
                        }
        ...
        return bl ? RaftEntity.Location.UNDER_WATER : null;
    }
    ...
    private void updateVelocity() {
        ...
        if (this.lastLocation == RaftEntity.Location.IN_AIR && this.location != RaftEntity.Location.IN_AIR && this.location != RaftEntity.Location.ON_LAND) {
            ...
            this.location = RaftEntity.Location.IN_WATER;
        } else {
            if (this.location == RaftEntity.Location.IN_WATER) {
                f = (this.waterLevel - this.getY()) / (double)this.getHeight();
                this.velocityDecay = 0.9F;
            } else if (this.location == RaftEntity.Location.UNDER_FLOWING_WATER) {
                e = -7.0E-4D;
                this.velocityDecay = 0.9F;
            } else if (this.location == RaftEntity.Location.UNDER_WATER) {
                f = 0.009999999776482582D;
                this.velocityDecay = 0.45F;
            } else if (this.location == RaftEntity.Location.IN_AIR) {
                this.velocityDecay = 0.9F;
            } else if (this.location == RaftEntity.Location.ON_LAND) {
    ...
    protected void writeCustomDataToNbt(NbtCompound nbt) {
        nbt.putString("Type", this.getRaftType().getName());
    }

    protected void readCustomDataFromNbt(NbtCompound nbt) {
        if (nbt.contains("Type", 8)) {
            this.setRaftType(RaftEntity.Type.getType(nbt.getString("Type")));
        }
        ...
        if (!this.hasVehicle()) {
            if (onGround) {
                if (this.fallDistance > 3.0F) {
                    if (this.location != RaftEntity.Location.ON_LAND) {
                        this.onLanding();
                        return;
                    }
                    ...
                    if (!this.world.isClient && !this.isRemoved()) {
                        ...
                        if (this.world.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
                            int i;
                            for(i = 0; i < 3; ++i) {
                                this.dropItem(this.getRaftType().getBaseBlock());
                            }
    ...
    public void setRaftType(RaftEntity.Type type) {
        this.dataTracker.set(RAFT_TYPE, type.ordinal());
    }

    public RaftEntity.Type getRaftType() {
        return RaftEntity.Type.getType((Integer)this.dataTracker.get(RAFT_TYPE));
    }
    ...
    public boolean isSubmergedInWater() {
        return this.location == RaftEntity.Location.UNDER_WATER || this.location == RaftEntity.Location.UNDER_FLOWING_WATER;
    }
    ...
    static {
        DAMAGE_WOBBLE_TICKS = DataTracker.registerData(RaftEntity.class, TrackedDataHandlerRegistry.INTEGER);
        DAMAGE_WOBBLE_SIDE = DataTracker.registerData(RaftEntity.class, TrackedDataHandlerRegistry.INTEGER);
        DAMAGE_WOBBLE_STRENGTH = DataTracker.registerData(RaftEntity.class, TrackedDataHandlerRegistry.FLOAT);
        RAFT_TYPE = DataTracker.registerData(RaftEntity.class, TrackedDataHandlerRegistry.INTEGER);
        LEFT_PADDLE_MOVING = DataTracker.registerData(RaftEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
        RIGHT_PADDLE_MOVING = DataTracker.registerData(RaftEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
        BUBBLE_WOBBLE_TICKS = DataTracker.registerData(RaftEntity.class, TrackedDataHandlerRegistry.INTEGER);
    }
    ...
    public static enum Type {
        ...
        public static RaftEntity.Type getType(int type) {
            RaftEntity.Type[] types = values();
        ...
        public static RaftEntity.Type getType(String name) {
            RaftEntity.Type[] types = values();
